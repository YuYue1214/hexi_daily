...
def _move_exported_excel_files() -> None:
    """
    将导出的Excel文件移动到hexi_timeframe文件夹
    """
    try:
        # 确保目标文件夹存在
        if not os.path.exists(timeframe_folder_path):
            os.makedirs(timeframe_folder_path)
            print(f"创建目标文件夹: {timeframe_folder_path}")
        ...
        for _, output_filename in EXPORT_TASKS:
            ...
            if os.path.exists(excel_path):
                try:
                    ...
                    # 生成新文件名：日期_原文件名
                    new_filename = f"{date_str}_{output_filename}"
                    # 目标路径
                    dest_path = os.path.join(timeframe_folder_path, new_filename)
                    ...
                    print(f"已移动文件: {output_filename} -> {new_filename}")
                    moved_count += 1
                except Exception as e:
                    print(f"移动文件失败 {output_filename}: {e}")

        print(f"移动Excel文件完成，共移动 {moved_count} 个文件")

    except Exception as e:
        print(f"移动Excel文件失败: {e}")
